...
// ─── Building geometry ────────────────────────────────────────────────────────
function Building({ rooms }: { rooms: any[] }) {
...
export default function Model3DPage() {
  ...
  // Sun position: faces south in N hemisphere, north in S hemisphere
  // Use lat to determine — positive lat = northern hemisphere = sun comes from S
  // Since we don't have lat in environmentalData, default sensibly (S for most users)
  const sunDir  = "S";
  ...
  // BIM export
  const handleExport = () => {
    const payload = { version:"ECO3D-BIM-1.0", plotId:id, rooms, exportedAt: new Date().toISOString(),
        ...
        <div className="flex-1 relative" style={{background:"radial-gradient(ellipse at 50% 30%, #0d2020 0%, #060e0e 70%)"}}>
          <Canvas camera={{position:[16,13,16],fov:48}} shadows gl={{antialias:true,alpha:false}}>
            ...
            <Suspense fallback={null}>
              {rooms.length > 0 ? <Building rooms={rooms}/> : <DefaultBuilding/>}
